...
class Pass4b:
  ...
  # To do: First find initializer expressions or variable
  # declarations
  ...
  #   spec_node = node.child(1)
  #   # If the type specifier is just a placeholder then type inference
  #   # is needed. Type inference requires that the type of the
  #   # initializer expression be computed.
  ...
  def expressionRoot (self, node: AstNode):
    ...
    print(f"Computed type is {expr_node.attribute('type')}")
  ...
  def name (self, node: AstNode):
    ...
    print(f"debug: In name function, printing {symbol}")
    if symbol:
