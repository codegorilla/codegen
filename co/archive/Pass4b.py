...
# 1. For each global variable declaration, determine if its type has
# been specified. If not, then it must have an initializer. Compute
# the type of its initializer expression, and then use it to infer
...
class Pass4b:
  ...
  def __init__ (self, root_node: AstNode, decl_list: List[AstNode]):
    ...
    self.decl_list: List[AstNode] = decl_list
    self.logger = Logger()
    self.unsigned_types = [
  ...
  def process (self):
    ...
    # Next we need to find any local variables that require type
    # inference. This could be combined with the next phase, which is
    # to search for expression roots, but for now do it as a separate
    # phase. This does mean that error messages may be out of order.
    # However, due to the compiler having multiple separate passes,
    # the error messages are already likely to be out of order. (They
    # may optionally be sorted later if we really care about that.)
    self.search1(self.root_node)
    self.logger.print()

  def search1 (self, node: AstNode):
    if node.kind == 'VariableDeclaration':
      self.variableDeclaration(node)
    else:
      for child_node in node.children:
        self.search1(child_node)

  # def search2 (self, node: AstNode):
  #   if node.kind == 'ExpressionRoot':
  #     self.expressionRoot(node)
  #   else:
  #     for child_node in node.children:
  #       self.search2(child_node)
  ...
  def variableDeclaration (self, node: AstNode):
    ...
    if spec_node.kind == 'AlphaType' and not spec_node.attribute('type'):
      init_node = node.child(2)
      self.expressionRoot(init_node)
      spec_node.set_attribute('type', init_node.attribute('type'))
      symbol: VariableSymbol = name_node.attribute('symbol')
